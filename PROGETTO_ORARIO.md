...
Creato dallo script `crea_template.py`, che legge il Word e i recapiti e precompila quello che può.
...
- Foglio Studenti: colonne **Comune, Indirizzo, Civico** (precompilate dai recapiti da `crea_template.py`).
...
2. ✅ `crea_template.py` → `input_orario.xlsx` precompilato (uso una tantum, per partire dai file esistenti).
...
crea_template.py        una tantum: Word + recapiti → input_orario.xlsx
...
# (facoltativo) rigenera il file di input dai file Word/Excel di partenza
.venv/bin/python crea_template.py
